...
function mergeInternal(base, rhs) {
    const dst = deepmerge_1.default(base, Object.assign(Object.assign({}, rhs), { rules: {} }));
...
function loadFromFile(filename) {
    ...
    try {
        ...
        json = utils_1.requireUncached(filename);
    }
    catch (err) {
...
class Config {
    ...
    static validate(configData, filename = null) {
        ...
        if (!valid) {
            throw new error_1.SchemaValidationError(filename, `Invalid configuration`, configData, config_json_1.default, (_a = validator.errors) !== null && _a !== void 0 ? _a : []);
        }
        if (configData.rules) {
            const normalizedRules = Config.getRulesObject(configData.rules);
            for (const [ruleId, [, ruleOptions]] of normalizedRules.entries()) {
                const cls = rules_1.default[ruleId];
                const path = `/rules/${ruleId}/1`;
                rule_1.Rule.validateOptions(cls, ruleId, path, ruleOptions, filename, configData);
            }
        }
    }
    /**
    ...
    resolve() {
        return new resolved_config_1.ResolvedConfig({
